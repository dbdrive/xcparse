...
struct AttachmentExportOptions {
    ...
    func screenshotDirectoryURL(_ inActivityIdentifier: Int, forBaseURL baseURL: Foundation.URL) -> Foundation.URL {
        if self.divideByInActivityIdentifier == true {
            let folderName = String(inActivityIdentifier)
            if self.xcresulttoolCompatability.supportsUnicodeExportPaths != true {
                let asciiFolderName = folderName.lossyASCIIString() ?? folderName
                return baseURL.appendingPathComponent(asciiFolderName, isDirectory: true)
            } else {
                ...
                for testableSummary in testableSummaries {
                    ...
                    for (testSummary, childActivitySummaries) in testableSummariesToTestActivity {
                        ...
                        let inActivityIdentifiers = filteredAttachments
                            .compactMap { $0.inActivityIdentifier }
                            .reduce(into: Set<Int>()) { $0.insert($1) }

                        for inActivityIdentifier in inActivityIdentifiers {

                            let inActivityIdentifierAttachments = filteredAttachments.filter { $0.inActivityIdentifier == inActivityIdentifier }
